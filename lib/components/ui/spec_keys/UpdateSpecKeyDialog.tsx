...
import { Prisma, type spec_groups } from "@/app/generated/prisma/client";
...
export function UpdateSpecKeyDialog({
  ...
  const [groupId, setGroupId] = useState<number>(specKey.group_id);
  const [groups, setGroups] = useState<spec_groups[] | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  // Trạng thái đóng/mở Popover chọn nhóm
  ...
  // Đồng bộ lại state từ specKey mỗi khi dialog được mở
  const handleOpenChange = (newOpen: boolean) => {
    setOpen(newOpen);
    if (newOpen) {
      setName(specKey.name);
      setGroupId(specKey.group_id);
  ...
  // Tìm tên nhóm đã chọn (nếu list chưa load kịp thì fallback tạm lấy từ relation specKey.spec_groups)
  const selectedGroup =
