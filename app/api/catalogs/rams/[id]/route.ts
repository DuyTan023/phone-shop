...
export async function PUT(req: NextRequest, { params }: routeContext) {
  ...
  try {
    ...
    const ram = await ramService.updateRam(numId, formatValue);
    return NextResponse.json<ApiResponse<rams>>(
      {
        success: true,
    ...
    console.error("Update ram API Error:", err); // Thêm dòng này để dễ debug terminal
    if (err instanceof Error && err.message === "NOT_FOUND") {
      return NextResponse.json<ApiResponse<null>>(
        { success: false, message: "Không tìm thấy ram để cập nhật" },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json<ApiResponse<null>>(
      { success: false, message: "Lỗi server khi cập nhật ram" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (err instanceof Error && err.message === "NOT_FOUND") {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
          message: "Không tìm thấy ram để xóa",
        },
        { status: 404 },
      );
    } else if (err instanceof Error && err.message === "RAM_HAS_PRODUCTS") {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
